...
- **Tipo:** `string | string[]`
  ...
  Por defecto, Vite rastreará todos sus archivos `.html` para detectar las dependencias que deben empaquetarse previamente (ignorando `node_modules`, `build.outDir`, `__tests__` y `coverage`). Si se especifica la opción de nivel superior [`input`](/config/shared-options#input) o `build.rolldownOptions.input`, Vite rastreará esos puntos de entrada en su lugar.
