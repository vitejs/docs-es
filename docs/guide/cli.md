...
Este comando inicia un servidor en el directorio de compilación (por defecto `dist`). Ejecuta `vite build` de antemano para asegurarte de que el directorio de compilación esté actualizado. Dependiendo del [`appType`](/config/shared-options#apptype) configurado en el proyecto, se utilizarán ciertos middlewares.
